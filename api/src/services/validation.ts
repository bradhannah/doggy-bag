...
export class ValidationServiceImpl implements ValidationService {
  ...
  validateBill(bill: Partial<Bill>): ValidationResult {
    ...
    } else if (!['monthly', 'bi_weekly', 'weekly', 'semi_annually'].includes(bill.billing_period)) {
    ...
    if (bill.billing_period === 'monthly') {
      ...
      const hasWeekdayRecurrence =
        bill.recurrence_week !== undefined && bill.recurrence_day !== undefined;
  ...
  validateIncome(income: Partial<Income>): ValidationResult {
    ...
    if (income.amount === undefined || income.amount === null || income.amount < 0) {
      errors.push('Amount must be zero or a positive number in cents');
    }
    ...
    if (income.billing_period === 'monthly') {
      ...
      const hasWeekdayRecurrence =
        income.recurrence_week !== undefined && income.recurrence_day !== undefined;
  ...
  validatePaymentSource(source: Partial<PaymentSource>): ValidationResult {
    ...
    if (source.track_payments_manually === true) {
      const debtTypes = ['credit_card', 'line_of_credit'];
      if (source.type && !debtTypes.includes(source.type)) {
        errors.push(
    ...
    if (source.pay_off_monthly === true && source.track_payments_manually === true) {
    ...
    if (
      ...
      (source.pay_off_monthly === true || source.track_payments_manually === true)
    ) {
    ...
    // is_investment is only valid for bank_account or investment type
    if (
      source.is_investment === true &&
      source.type &&
      !['bank_account', 'investment'].includes(source.type)
    ) {
  ...
  validatePaymentSourceType(type: string): boolean {
    return ['bank_account', 'credit_card', 'line_of_credit', 'cash', 'investment'].includes(type);
  }
