...
function extractMonthAndId(
  url: string,
  type: 'bills' | 'incomes'
): { month: string | null; id: string | null } {
    ...
    try {
      ...
      if (!month || !instanceId || !occurrenceId) {
        return new Response(
          JSON.stringify({
            error:
              'Invalid URL. Expected /api/months/YYYY-MM/bills/:instanceId/occurrences/:occurrenceId',
          }),
          {
            headers: { 'Content-Type': 'application/json' },
            status: 400,
    ...
    try {
      ...
      if (!month || !instanceId || !occurrenceId) {
        return new Response(
          JSON.stringify({
            error:
              'Invalid URL. Expected /api/months/YYYY-MM/incomes/:instanceId/occurrences/:occurrenceId',
          }),
          {
            headers: { 'Content-Type': 'application/json' },
            status: 400,
  ...
  return async (request: Request) => {
    try {
      ...
      const match = url.pathname.match(
        /\/api\/months\/(\d{4}-\d{2})\/bills\/([^/]+)\/occurrences\/([^/]+)\/close/
      );
      const month = match ? match[1] : null;
      ...
      if (!month || !instanceId || !occurrenceId) {
        return new Response(
          JSON.stringify({
            error:
              'Invalid URL. Expected /api/months/YYYY-MM/bills/:instanceId/occurrences/:occurrenceId/close',
          }),
          {
            headers: { 'Content-Type': 'application/json' },
            status: 400,
...
export function createBillOccurrenceHandlerReopen() {
  return async (request: Request) => {
    try {
      const url = new URL(request.url);
      const match = url.pathname.match(
        /\/api\/months\/(\d{4}-\d{2})\/bills\/([^/]+)\/occurrences\/([^/]+)\/reopen/
      );
      const month = match ? match[1] : null;
      const instanceId = match ? match[2] : null;
      const occurrenceId = match ? match[3] : null;
      ...
      if (!month || !instanceId || !occurrenceId) {
        return new Response(
          JSON.stringify({
            error:
              'Invalid URL. Expected /api/months/YYYY-MM/bills/:instanceId/occurrences/:occurrenceId/reopen',
          }),
          {
            headers: { 'Content-Type': 'application/json' },
            status: 400,
...
export function createIncomeOccurrenceHandlerClose() {
  return async (request: Request) => {
    try {
      const url = new URL(request.url);
      const match = url.pathname.match(
        /\/api\/months\/(\d{4}-\d{2})\/incomes\/([^/]+)\/occurrences\/([^/]+)\/close/
      );
      const month = match ? match[1] : null;
      const instanceId = match ? match[2] : null;
      const occurrenceId = match ? match[3] : null;
      ...
      if (!month || !instanceId || !occurrenceId) {
        return new Response(
          JSON.stringify({
            error:
              'Invalid URL. Expected /api/months/YYYY-MM/incomes/:instanceId/occurrences/:occurrenceId/close',
          }),
          {
            headers: { 'Content-Type': 'application/json' },
            status: 400,
...
export function createIncomeOccurrenceHandlerReopen() {
  return async (request: Request) => {
    try {
      const url = new URL(request.url);
      const match = url.pathname.match(
        /\/api\/months\/(\d{4}-\d{2})\/incomes\/([^/]+)\/occurrences\/([^/]+)\/reopen/
      );
      const month = match ? match[1] : null;
      const instanceId = match ? match[2] : null;
      const occurrenceId = match ? match[3] : null;
      ...
      if (!month || !instanceId || !occurrenceId) {
        return new Response(
          JSON.stringify({
            error:
              'Invalid URL. Expected /api/months/YYYY-MM/incomes/:instanceId/occurrences/:occurrenceId/reopen',
          }),
          {
            headers: { 'Content-Type': 'application/json' },
            status: 400,
...
export function createBillAdhocOccurrenceHandler() {
  return async (request: Request) => {
    try {
      const url = new URL(request.url);
      const match = url.pathname.match(
        /\/api\/months\/(\d{4}-\d{2})\/bills\/([^/]+)\/occurrences$/
      );
      const month = match ? match[1] : null;
      const instanceId = match ? match[2] : null;
...
export function createIncomeAdhocOccurrenceHandler() {
  return async (request: Request) => {
    try {
      const url = new URL(request.url);
      const match = url.pathname.match(
        /\/api\/months\/(\d{4}-\d{2})\/incomes\/([^/]+)\/occurrences$/
      );
      const month = match ? match[1] : null;
      const instanceId = match ? match[2] : null;
