<script lang="ts">
  ...
  function showNotes(bill: BillWithContribution) {
    notesModalTitle = `${bill.name} - Notes`;
    notesModalContent = bill.metadata?.notes || '';
    notesModalOpen = true;
  }
  ...
  function hasMetadata(bill: BillWithContribution): boolean {
    return !!(
      bill.metadata?.bank_transaction_name ||
      bill.metadata?.account_number ||
      bill.metadata?.account_url ||
      bill.metadata?.notes
    );
  }
    ...
    {#each visibleGroups as group (group.category?.id || 'uncategorized')}
      ...
      {:else}
        {#each group.bills as bill (bill.id)}
          <div
            class="bill-row"
            class:has-metadata={hasMetadata(bill)}
            ...
            <div class="col-name">
              ...
              {#if hasMetadata(bill)}
                <div class="metadata-row">
                  ...
                  {/if}
                  {#if bill.metadata?.account_number}
                  ...
                  {/if}
                  {#if bill.metadata?.account_url}
                    <a
                      href={bill.metadata.account_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      class="metadata-link"
                      title={bill.metadata.account_url}
                    ...
                    >
                      ...
                      {shortenUrl(bill.metadata.account_url)}
                    </a>
                  {/if}
                  {#if bill.metadata?.notes}
                    <button
                      class="metadata-notes-btn"
                      title="View notes"
                      on:click|stopPropagation={() => showNotes(bill)}
                    >
                      <svg
                        width="10"
                        height="10"
