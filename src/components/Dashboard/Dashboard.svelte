<script lang="ts">
  ...
  function sumOccurrencePayments(item: any): number {
    const occPayments = (item.occurrences || []).reduce((oSum: number, occ: any) => 
      oSum + (occ.payments || []).reduce((pSum: number, p: any) => pSum + p.amount, 0), 0);
    const legacyPayments = (item.payments || []).reduce((pSum: number, p: any) => pSum + p.amount, 0);
    return occPayments + legacyPayments;
  }
  
  // Calculate totals from instances - use expected_amount and occurrence-based payments
  $: billsExpected = $billInstances.reduce((sum, b) => sum + ((b as any).expected_amount || b.amount || 0), 0);
  ...
  $: incomeExpected = $incomeInstances.reduce((sum, i) => sum + ((i as any).expected_amount || i.amount || 0), 0);
  ...
  async function loadAdjacentMonths() {
    ...
    try {
      ...
      if (prevRes && prevRes.ok) {
        ...
        const billsExp = data.bill_instances?.reduce((s: number, b: any) => s + (b.expected_amount || b.amount || 0), 0) || 0;
        const billsPd = data.bill_instances?.reduce((s: number, b: any) => s + sumOccurrencePayments(b), 0) || 0;
        const incExp = data.income_instances?.reduce((s: number, i: any) => s + (i.expected_amount || i.amount || 0), 0) || 0;
        const incRec = data.income_instances?.reduce((s: number, i: any) => s + sumOccurrencePayments(i), 0) || 0;
        ...
        prevMonthData = { month: prevMonth, exists: false, isReadOnly: false, leftover: 0, billsProgress: 0, incomeProgress: 0 };
      }
      ...
      if (nextRes && nextRes.ok) {
        ...
        nextMonthData = { month: nextMonth, exists: false, isReadOnly: false, leftover: 0, billsProgress: 0, incomeProgress: 0 };
      }
    } catch (error) {
    ...
    <section class="adjacent-months">
      ...
      <button class="month-card prev" on:click={() => viewMonth(prevMonth)}>
        ...
        {#if prevMonthData?.exists}
          ...
          <div class="card-leftover" class:positive={prevMonthData.leftover >= 0} class:negative={prevMonthData.leftover < 0}>
      ...
      <button class="month-card next" on:click={() => viewMonth(nextMonth)}>
        ...
        {#if nextMonthData?.exists}
          ...
          <div class="card-leftover" class:positive={nextMonthData.leftover >= 0} class:negative={nextMonthData.leftover < 0}>
      ...
      <section class="attention-section">
        ...
        <div class="attention-list">
          {#each attentionItems as item}
            <div class="attention-item" class:lock={item.type === 'lock'} class:due-soon={item.type === 'due_soon'}>
