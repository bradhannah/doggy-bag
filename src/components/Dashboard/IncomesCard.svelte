<script lang="ts">
  ...
  async function saveEdit(id: string) {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/months/${month}/incomes/${id}`, {
        method: 'PUT',
  ...
  async function resetToDefault(id: string) {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/months/${month}/incomes/${id}/reset`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
        ...
        <li class="income-item" class:editing={editingId === income.id} class:modified={!income.is_default} class:paid={income.is_paid}>
          ...
          {:else}
            ...
            <div class="income-actions">
              ...
              {#if !income.is_default}
                <button class="reset-btn" on:click={() => resetToDefault(income.id)} disabled={saving}>
                  Reset
                </button>
              {/if}
            </div>
